...
def _create_user(db, company, username="admin", password="123456", question="Qual sua comida favorita?", answer="Pizza"):
...
def test_recover_redirects_to_answer_step_when_question_exists(client, db, company):
    _create_user(db, company)
...
def test_recover_rejects_unknown_username(client, db, company):
    _create_user(db, company)
...
def test_recover_answer_resets_password_with_correct_answer(client, db, company):
    _create_user(db, company)
...
def test_recover_answer_rejects_wrong_answer(client, db, company):
    _create_user(db, company)
