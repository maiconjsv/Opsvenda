...
@login_required
def index():
    sales = scoped_query(Sale).order_by(Sale.sale_date.desc()).limit(200).all()
...
@login_required
def create():
    products = scoped_query(Product).filter_by(active=True).order_by(Product.name).all()
    profiles = scoped_query(MarginProfile).filter_by(active=True).order_by(MarginProfile.name).all()
...
def _create_manual_sale(form):
    ...
    product = scoped_get_or_none(Product, product_id)
    if product is None:
        return "Produto não encontrado.", None

    margin_profile = scoped_get_or_none(MarginProfile, margin_profile_id)
